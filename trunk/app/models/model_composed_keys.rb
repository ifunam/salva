...
class ModelComposedKeys < ActiveRecord::Base
  ...
  class << self
    ...
    def set_primary_keys(*keys)
      ...
      self.reserved_attributes = %w(created_on updated_on moduser_id)
    end
    ...
    def find_every_composed_keys(options)
      grouped_records = find_every({:select => self.primary_keys.join(','), :group => self.primary_keys.join(','),
                                     :conditions => options[:conditions], :include => options[:include],
                                     :joins => options[:joins] })
  ...
  def my_update
    ...
    # Here is missing a better way to destroy an unselected records,
    # and Yes, I know, It is a very ugly hack but It works!
    record = self.class.find(self.class.ids)
    saved = {}
    array_keys.each { |k| saved[k] = record.attributes_before_type_cast[k] }
    unselected = {}
    saved.keys.each { |k|  unselected[k] = saved[k] - attributes[k]  if saved[k].compact.size > 0 }
    (0..get_maxsize_from_array_attributes(unselected)).each { |i|
      conditions = self.class.set_conditions(self.class.primary_keys + unselected.keys, self.class.ids.split(':') + unselected.keys.collect { |key| unselected[key][i] })
  ...
  def get_maxsize_from_array_attributes(myattributes)
    array = myattributes.values.collect! { |value| value.size if value.is_a?(Array) }.compact
