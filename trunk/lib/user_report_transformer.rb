...
class UserReportTransformer
  ...
  def as_html(data)
    ...
    data.each do |hash|
      ...
      body << html_paragraph(hash[:data])  if hash.has_key?(:data)
      index += 1
    ...
    html_table_of_content(toc) + body
  end
  ...
  def html_table_of_content(toc) 
    n = 0
    internal_link('toc') + "\n" + table("table(toc).\n" + '|' + toc.collect { |section| html_table_of_content_section(section, (n <= 4 ?  n += 1 : n = 1)) }.join("|") + '|') + "\n"
  end

  def html_table_of_content_section(section, n)
