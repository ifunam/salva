module Stackcontroller
  def options_for_next_controller(model, controller, action, form_params, stack_params, next_action='new')
  ...
  def set_options(stack_params, form_params)
    if stack_params =~ /:/
      [ stack_params.split(':')[0], stack_params.split(':')[0]+'_id',  form_params[stack_params.split(':')[1].to_sym]  ]
    elsif stack_params =~ /\,/
