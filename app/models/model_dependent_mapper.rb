class ModelDependentMapper
  ...
  def set_attributes(params)
    [@owner_klass, @sequence].flatten.each do |klass|
      k = ActiveSupport::Inflector.underscore(klass).to_sym
      @records[k] = record_loader(klass, params[k]) unless params[k].nil?
    end
    @owner = @records[ActiveSupport::Inflector.underscore(@owner_klass).to_sym]
  end

  def set_user(user_id)
    @owner.user_id = user_id if !@owner.nil? and @owner.has_attribute? :user_id
  end
