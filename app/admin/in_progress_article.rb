...
ActiveAdmin.register UserArticle, :as => "InProgressArticle"  do
  ...
  controller do
    def scoped_collection
      UserArticle.inprogress
    end
  end

  index :title => 'Artículos enviados o aceptados' do
  ...
  filter :articlestatus_id, :collection => proc { Articlestatus.where("id != 3").all }, :label => 'Status', :as => :select
