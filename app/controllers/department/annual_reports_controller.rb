class Department::AnnualReportsController < Department::ApplicationController
  respond_to :html, :js
  layout 'department'
  ...
  def index
    if params[:reset] or params[:button]
      ...
      params[:search][:year] = Time.now.year-1
    end
    if current_user.admin? then
      @documents = Document.annual_reports.order("users.author_name").paginated_search(params)
      return
    else
      name = params[:search].nil? ? nil : params[:search][:name]
      jobposition = params[:search].nil? ? nil : params[:search][:jobposition]
      year = params[:search].nil? ? Time.now.year-1 : params[:search][:year]
      respond_with(@documents = Document.annual_reports.department_documents(current_user.head_adscription_id,year,name,jobposition).page(params[:page] || 1).per(20))
      return
    end
  end
  ...
  def download_pdf
    ...
    if current_user.admin? or user.id == current_user.id or user.user_incharge_id == current_user.id or user.adscription_id == current_user.head_adscription_id
      send_file("#{Rails.root}/app/files/annual_reports/#{year}/#{user.login}.pdf",
              filename: "#{user.login}_#{year}_report.pdf",
