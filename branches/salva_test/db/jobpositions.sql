...
DECLARE
BEGIN
        IF (OLD.jobpositioncategory_id = NEW.jobpositioncategoy_id) AND (OLD.contracttype_id = NEW.contracttype_id)  THEN
                RETURN NEW;
        END IF;
        INSERT INTO jobpositionlog (user_id, old_jobpositioncategory_id, old_contracttype_id, institution_id, startyear, startmonth, endyear, endmonth,  moduser_id  )
                VALUES (OLD.id, OLD.user_id, OLD.jobpositioncategory_id, OLD.contracttype_id, OLD.institution_id, OLD.startyear, OLD.startmonth, OLD.endyear, OLD.endmonth,  OLD.moduser_id);
        RETURN NEW;
END;
' LANGUAGE 'plpgsql';
